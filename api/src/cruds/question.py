...
async def get_questions(db: AsyncSession, skip: int = 0, limit: int = 100) -> List[Question]:
    result = await db.execute(
        select(Question).options(selectinload(Question.user)).order_by(Question.created_at.desc()).offset(skip).limit(limit)
    )
    return result.scalars().all()

async def get_question(db: AsyncSession, question_id: UUID) -> Question | None:
    result = await db.execute(
        select(Question).options(selectinload(Question.user)).where(Question.id == question_id)
    )
